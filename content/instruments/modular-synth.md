...
Modular Synth 是收錄在《隔壁家的世界聲音旅圖》資料庫中的鍵盤樂器。目前資料庫將它歸在「電鳴」脈絡下，主要可從電子與錄音室延伸、控制／取樣／合成與聲音地景來認識。

初步地區標註為「全球」。這個欄位仍屬企劃初稿，後續可依博物館、百科條目、民族音樂學資料或演奏者資料逐篇核對。

## 歷史背景

Modular Synth 的歷史背景目前先以資料庫分類建立閱讀入口。它位於「12｜按鍵打開的機械房間」的脈絡中，代表這件樂器可以和氣息、材料、身體動作、地方儀式或現代舞台之間的關係一起閱讀。
...
- **資料狀態**：承接553條目地域欄
### 讀者導覽
閱讀 Modular Synth 時，可以先從「電子與錄音室延伸」與「控制／取樣／合成」切入，觀察它如何把材料、手勢與共鳴連在一起。
