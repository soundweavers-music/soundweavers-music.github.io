...
Synthesizer 是收錄在《隔壁家的世界聲音旅圖》資料庫中的鍵盤樂器。目前資料庫將它歸在「電鳴」脈絡下，主要可從合成器與取樣器、電子震盪／取樣與聲音地景來認識。

初步地區標註為「歐洲／美國／全球現代錄音室」。這個欄位仍屬企劃初稿，後續可依博物館、百科條目、民族音樂學資料或演奏者資料逐篇核對。

## 歷史背景

Synthesizer 的歷史背景目前先以資料庫分類建立閱讀入口。它位於「12｜按鍵打開的機械房間」的脈絡中，代表這件樂器可以和氣息、材料、身體動作、地方儀式或現代舞台之間的關係一起閱讀。
...
- **資料狀態**：系統初稿推定，需逐篇來源核對
### 讀者導覽
閱讀 Synthesizer 時，可以先從「合成器與取樣器」與「電子震盪／取樣」切入，觀察它如何把材料、手勢與共鳴連在一起。
