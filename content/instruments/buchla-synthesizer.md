...
image_spec: "西岸合成器思維代表"
source_basis: "10_補充後總清單＋553條目地域欄"
database_status: "建議新增"
database_note: "西岸合成器思維代表"
---
## 介紹

Buchla Synthesizer 是收錄在《隔壁家的世界聲音旅圖》資料庫中的鍵盤樂器。目前資料庫將它歸在「電鳴／機械／混合」脈絡下，主要可從模組合成器、控制電壓與聲音地景來認識。

初步地區標註為「歐洲／美國／全球現代錄音室」。這個欄位仍屬企劃初稿，後續可依博物館、百科條目、民族音樂學資料或演奏者資料逐篇核對。

## 歷史背景

Buchla Synthesizer 的歷史背景目前先以資料庫分類建立閱讀入口。它位於「12｜按鍵打開的機械房間」的脈絡中，代表這件樂器可以和氣息、材料、身體動作、地方儀式或現代舞台之間的關係一起閱讀。

如果要補寫成正式百科條目，建議優先查證三個方向：一是它最早出現或被記錄的地區，二是模組合成器的形制演變，三是控制電壓如何影響音色、用途與傳播。
...
- **資料狀態**：系統初稿推定，需逐篇來源核對
### 讀者導覽
閱讀 Buchla Synthesizer 時，可以先從「模組合成器」與「控制電壓」切入，觀察它如何把材料、手勢與共鳴連在一起。
